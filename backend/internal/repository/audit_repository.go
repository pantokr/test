...
func (r *AuditRepository) UpdateUserAccountLogoutTime(loginID string) error {
	const query = `
...
func (r *AuditRepository) UpdateUserAccountLoginFail(loginID string) error {
	const query = `
		UPDATE user_account
...
func (r *AuditRepository) UpdateUserAccountLoginSuccess(loginID string) error {
	const query = `
		UPDATE user_account
		SET pw_fail_count = 0,
...
func (r *AuditRepository) InsertLoginFailHistory(failCode, loginID, clientIP, serverIP string) error {
	const query = `
...
func (r *AuditRepository) SelectLoginFailHistoryAll() ([]response.LoginFailResponse, error) {
	const query = `
		SELECT 
			login_id, 
			IFNULL(DATE_FORMAT(login_time, '%Y-%m-%d %H:%i:%s'), '') AS login_time,
