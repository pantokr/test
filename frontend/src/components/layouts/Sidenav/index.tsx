...
import { useAuth, useDevice, useSidenav, useThemeSettings } from "@/context";
import { RouteItem } from "@/routes/types";
import { hasPermission, isPublicRoute } from "@/utils/route";
...
const Sidenav: React.FC<Omit<SidenavProps, "color">> = ({
  ...
  const renderRoutes = (routes: RouteItem[]): ReactNode =>
    routes.map((route) => {
      ...
      if (!hasPermission(user?.permission, route)) {
        return null;
      }
      ...
      if (route.collapse) {
        const filteredCollapse = route.collapse.filter((subRoute) => {
          ...
          return (
            !(subRoute.route && isPublicRoute(subRoute.route)) &&
            user &&
            hasPermission(user.permission, subRoute)
          );
        });
