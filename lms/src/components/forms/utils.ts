import { UserRegistration, UserUpdate } from "@/api/types/user";
 ...
 */
export const validatePasswdUpdatedForm = (formData: {
  password: string;
...
export const validateUserUpdateForm = (
  formData: UserUpdate,
  confirmPwd: string
): { valid: boolean; error?: string } => {
  let error: string | null = null;
  ...
  if (formData.newPasswd) {
    error = validatePassword(formData.newPasswd);
    if (error) return { valid: false, error };

    error = validateConfirmPassword(formData.newPasswd, confirmPwd);
    if (error) return { valid: false, error };
  }
